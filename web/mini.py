...
# An initData payload older than a day is rejected (replay window).
...
async def mini_state(request: Request) -> dict:
    ...
    tg_id = _user(request)
    markets = []
    for m in ledger.open_markets(6):
            ...
            markets.append(
                {
                    ...
                    "close_at": view["close_at"],
                    "traders": view["traders"],
                    "options": [
                        {"index": o["index"], "label": o["label"], "price_pct": o["price_pct"]}
                        for o in view["options"]
                    ],
                }
            )
    bets = []
    for b in ledger.bets_by_status("open", 6):
        totals = ledger.bet_totals(int(b["id"]))
        import json as _json
        ...
        bets.append(
            {
                "id": int(b["id"]),
                "question": b["question"],
                "creator": int(b["creator"]),
                "pot_usdc": _fmt(pot),
                "options": [
                    {
                        "index": i,
                        "label": lbl,
                        "pool_usdc": _fmt(totals.get(i, 0)),
                        "chance_pct": round(100 * totals.get(i, 0) / pot, 1) if pot else 0.0,
                    }
    ...
    history = [
        ...
        for r in ledger.history(tg_id, 10)
    ]
    top = [{"username": r.get("username") or f"id{r['tg_id']}", "total_usdc": _fmt(r["total_micro"])} for r in ledger.leaderboard(5)]
    lang = ledger.get_settings(tg_id).get("lang", "ru")
    return {
        "tg_id": tg_id,
        "username": ledger.username_of(tg_id),
        "balance_usdc": float(ledger.balance(tg_id)),
        "deposit_address": str(base.hot_wallet()),
...
async def mini_tip(body: TipBody, request: Request) -> dict:
    ...
    target = ledger.find_by_username(to)
    ...
    if not ledger.transfer(tg_id, target, micro):
        raise HTTPException(400, "insufficient balance")
    return {"ok": True, "new_balance": float(ledger.balance(tg_id))}
...
async def mini_trade(body: TradeBody, request: Request) -> dict:
    ...
    if micro <= 0:
        raise HTTPException(400, "amount must be positive")
    status, info = ledger.buy_shares(body.market_id, tg_id, body.option, micro)
    if status != "ok":
        raise HTTPException(400, _ERR_MSG.get(status, status))
    bal = float(ledger.balance(tg_id))
    pos = ledger.user_market_position(body.market_id, tg_id) or {}
...
async def mini_betplace(body: BetPlaceBody, request: Request) -> dict:
    ...
    if micro <= 0:
        raise HTTPException(400, "amount must be positive")
    res = ledger.place_bet(body.bet_id, tg_id, body.option, micro)
    if res != "ok":
...
async def mini_create(body: CreateBody, request: Request) -> dict:
    ...
    if body.kind == "market":
        mid = ledger.create_market(tg_id, question, options, close_at=close_at)
        return {"ok": True, "id": mid}
    if body.kind == "bet":
        bid = ledger.create_bet(tg_id, question, options, close_at=close_at)
...
async def mini_lang(body: LangBody, request: Request) -> dict:
    ...
    ledger.set_setting(tg_id, "lang", body.lang)
